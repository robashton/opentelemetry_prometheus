...
export(Callback) ->
    ...
    maps:fold(fun({Name, LabelSet}, Record, Acc) ->
                      [Callback(view_data_to_mf(Name, LabelSet, Record)) | Acc]
              end, [], Records).
...
rows_to_metrics(otel_metric_aggregator_last_value, [{Tags, {Value, _Time}}]) ->
    prometheus_model_helpers:gauge_metrics([{Tags, Value}]);
rows_to_metrics(otel_metric_aggregator_sum, Rows) ->
    prometheus_model_helpers:counter_metrics(Rows);
rows_to_metrics(otel_metric_aggregator_mmsc, [{Tags, {Min, Max, Sum, Count}}]) ->
    prometheus_model_helpers:histogram_metric(
      Tags, [{0, Min}, {1, Max}], Count, Sum).

to_prom_type(otel_metric_aggregator_last_value) ->
    gauge;
to_prom_type(otel_metric_aggregator_sum) ->
    counter;
to_prom_type(otel_metric_aggregator_mmsc) ->
    summary.
